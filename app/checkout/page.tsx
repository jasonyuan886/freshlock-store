...
export default function CheckoutPage() {
  ...
  // Free-shipping threshold is evaluated on the pre-discount subtotal (matches
  // the server), so applying a coupon never strips away free shipping.
  const shipping = totalPrice >= FREE_SHIPPING_THRESHOLD ? 0 : SHIPPING_FEE_UNDER;
  const [paymentMethod, setPaymentMethod] = useState('paypal');
  const [processing, setProcessing] = useState(false);
  const [error, setError] = useState('');
  // Promo code: display-only client state. The server re-validates the code
  // and computes the real charge independently in /api/paypal.
  const [couponInput, setCouponInput] = useState('');
  const [appliedCoupon, setAppliedCoupon] = useState<{ code: string; percentOff: number; description: string } | null>(null);
  const [couponChecking, setCouponChecking] = useState(false);
  const [couponError, setCouponError] = useState('');
  const discount = appliedCoupon ? +(totalPrice * appliedCoupon.percentOff).toFixed(2) : 0;
  const total = +(totalPrice - discount + shipping).toFixed(2);
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (data.valid) {
        setAppliedCoupon({ code: data.code, percentOff: data.percentOff, description: data.description });
      } else {
              ...
              <div className="border-t pt-4 mt-4">
                {appliedCoupon ? (
                  <div className="flex items-center justify-between bg-green-50 border border-green-300 rounded-lg px-3 py-2">
                    <span className="text-xs text-green-700 font-medium">✅ {appliedCoupon.description}</span>
                    <button
                      type="button"
                  ...
                  <div>
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 min-w-0 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-transparent outline-none"
                      />
                      <button
                        type="button"
                        onClick={handleApplyCoupon}
                        disabled={couponChecking || !couponInput.trim()}
                        className="px-4 py-2 text-sm font-medium border-2 border-primary text-primary rounded-lg hover:bg-primary hover:text-white transition disabled:opacity-50 disabled:cursor-not-allowed"
                      >
